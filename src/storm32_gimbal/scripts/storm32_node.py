...
if __name__ == "__main__":
    ...
    if version:
        ...
        camera_pub = rospy.Publisher("~camera_orientation", QuaternionStamped, queue_size=1)
        controller_pub = rospy.Publisher("~controller_orientation", QuaternionStamped, queue_size=1)

        restart_srv = rospy.Service("~restart", Trigger, restart_controller)
	rospy.loginfo("ustawlem publisherow")
        # Setup periodic callback for orientation publisher
        pub_timer = rospy.Timer(rospy.Duration(0.5), pub_timer_callback)	#0.01 na 1
	rospy.loginfo("odpaliem timer")
        # Spin forever
        rospy.spin()
    else:
