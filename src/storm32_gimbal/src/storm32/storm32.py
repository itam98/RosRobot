...
class Storm32(object):
    ...
    _MSG_HEADER_IN = 0xFD	#3: ZMIENILEM HEADER WIADOMOSCI PRZYCHODZACEJ z 0xFB
    _MSG_HEADER_OUT = 0xFD	#2:zmienilem header z 0xFA
    ...
    _CMD_HEADER_IN = 0xFD		#7?: zmienilem tez tu na 0xFD
    _CMD_HEADER_OUT = 0xFD
    ...
    def _send_msg(self, cmd, data=[]):
        ...
        msg_len = len(data)
	if cmd == 0x96:
		msg = [msg_len,0x00, 0x47, 0x43, cmd]	#8: dodalem tu 3 bajty jesli cmd_ack
	else:        
		msg = [msg_len,0x00, 0x52, 0x43, cmd]	#1: dodalem tu 3 bajty
        msg.extend(data)
    ...
    def __get_msg(self, cmd):
        ...
        return: response from thae gimbal controller as an "int" list, empty
        list of timeout.
        ...
        while (time() - start_time) < self._MSG_READ_TIMEOUT:
            ...
            for i in range(0, len(self.buff) - 6):	#9: tutaj tez zmienilem 3 na 6

                # if header found and command is right, or command is ACK
                if ord(self.buff[i]) 	== self._MSG_HEADER_IN and (
                    (ord(self.buff[i + 5]) == cmd or
                     (ord(self.buff[i + 5]) == self._CMD_ACK))):   #4: zmienilem z 2 na 5, szukaj o 3 dalej

                    # Check if all bytes of the message is here
                    end_of_msg = i + ord(self.buff[i + 1]) + 8	#5: zmiana 5 na 8, wiadomosc jest o 3 bajty dluzsza
                    if end_of_msg <= len(self.buff):
                        ...
                        # Exception if needed.
                        if msg[4] == self._CMD_ACK:		#6: do kazej wartosci w msg[x] dodalem 3
                            if msg[5] == self._CMD_ACK_OK:
                                return msg
                            if msg[5] == self._CMD_ACK_ERR_ACCESS_DENIED:
                                raise ACKAccessDeniedError()
                            if msg[5] == self._CMD_ACK_ERR_FAILD:
                                raise ACKFailError()
                            if msg[5] == self._CMD_ACK_ERR_PAYLOAD_LEN:
                                raise ACKPayloadLen()
                            if msg[5] == self._CMD_ACK_ERR_TIMEOUT:
                                raise ACKTimeout()
                            if msg[5] == self._CMD_ACK_ERR_NOT_SUPPORTED:
